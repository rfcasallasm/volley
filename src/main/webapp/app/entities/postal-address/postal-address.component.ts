...
export class PostalAddressComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.postalAddressService
            ...
            .subscribe(
                (res: HttpResponse<IPostalAddress[]>) => this.paginatePostalAddresses(res.body, res.headers),
    ...
    trackId(index: number, item: IPostalAddress) {
        return item.id;
    }

    registerChangeInPostalAddresses() {
        this.eventSubscriber = this.eventManager.subscribe('postalAddressListModification', response => this.reset());
    }
    ...
    protected paginatePostalAddresses(data: IPostalAddress[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.postalAddresses.push(data[i]);
        }
    }
